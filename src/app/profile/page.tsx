...
const EditProfile = () => {
  ...
  const { mutate: updateProfileMutation } = useUpdateProfile({
    onSuccess: () => {
      router.push("/");
      toast.success(translate("Profile edited successfully!", "Profil modifié avec succès!", "تم تعديل الملف الشخصي بنجاح!"));
    },
  });
  ...
  const mutation = useUpdateProfilePicture({
    onSuccess: () => {
      toast.success(translate("Profile picture updated successfully!", "Photo de profil mise à jour avec succès!", "تم تحديث الصورة الشخصية بنجاح!"));
      setUploading(false);
      refetchDataUpdate();
      refetchProfile();
    },
    onError: () => {
      toast.error(translate("Failed to update profile picture. Please try again.", "Échec de la mise à jour de la photo de profil. Veuillez réessayer.", "فشل تحديث الصورة الشخصية. يرجى المحاولة مرة أخرى."));
      setUploading(false);
    },
  });
      ...
      <Container>
        <div className="w-full overflow-x-hidden rounded-xl bg-bgPrimary p-4">
          <Text font={"bold"} size={"4xl"}>
            {translate("Edit Profile", "Modifier le profil", "تعديل الملف الشخصي")}
          </Text>
        ...
        priority={true}
              ...
              <div className="relative">
                <button
                  ...
                  aria-label={translate("Edit Profile Picture", "Modifier la photo de profil", "تعديل الصورة الشخصية")}
          ...
          <div className="m-auto w-4/5">
            <div className="flex flex-col md:flex-row gap-8">
              <div>
                <a href="/profile" className="text-xl text-primary underline">
                  {translate("Personal Info.", "Infos personnelles", "المعلومات الشخصية")}
                </a>
              </div>
              <div>
                <a href="/password" className="text-xl">
                  {translate("Change Password", "Changer le mot de passe", "تغيير كلمة المرور")}
                </a>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <label htmlFor="name">{translate("Name", "Nom", "الاسم")}</label>
                <Input
                  name="name"
                  placeholder={translate("Enter name", "Entrez le nom", "أدخل الاسم")}
                  theme="transparent"
                  border="gray"
                  value={name}
              ...
              <div>
                <label htmlFor="phone">{translate("Phone Number", "Numéro de téléphone", "رقم الهاتف")}</label>
                <Input
                  type="tel"
                  placeholder={translate("Phone Number", "Numéro de téléphone", "رقم الهاتف")}
                  pattern="^\\+?[1-9]\d{1,14}$"
                  theme="transparent"
                  border="gray"
                  value={phone}
              ...
              <div>
                <label htmlFor="gender">{translate("Gender", "Genre", "الجنس")}</label>
                <select
                  name="gender"
                  id="gender"
                ...
                >
                  <option value="MALE">{translate("Male", "Homme", "ذكر")}</option>
                  <option value="FEMALE">{translate("Female", "Femme", "أنثى")}</option>
                </select>
              </div>
            </div>
          </div>
