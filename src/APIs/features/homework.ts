...
export const fetchSessionMaterials = async (sessionId: string, studentId: string): Promise<any> => {
  const response = await axiosInstance.get<any>(
    `/api/v1/parent/session-materials?sessionId=${sessionId}&studentId=${studentId}`
  );
  return response.data;
};
