 ...
 */
const Box: React.FC<BoxProps> = ({ children, className = "", shadow = "sm", border = "none", width = 'full', ...props }) => {
  return (
    <div
      className={`w-${width} rounded-xl bg-bgPrimary p-4 shadow-${shadow} ${border !== "none" ? `border ${border}` : ""} ${className}`}
      {...props}
    >
      {children}
    </div>
